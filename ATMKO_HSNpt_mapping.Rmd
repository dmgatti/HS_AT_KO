...
```{r gwas_brain}
  ...
  pheno$brain_num = as.numeric(pheno$brain)
  
  gwas = scan1snps(genoprobs = probs, pheno = pheno[,'brain_num',drop = F], 
                  map = map, kinship = K, keep_all_snps = TRUE,
                  query_func = snp_func, cores = 8)

  saveRDS(gwas, file = file.path(results_dir, 'brain_gwas.rds'))

  png(file.path(fig_dir, 'brain_gwas.png'), width = 2000, height = 1600,
                res = 300)
  plot_snpasso(gwas$lod, gwas$snpinfo, main = 'Brain Score')
...
#    png(file.path(fig_dir, paste0('brain_gwas_chr', chr,'.png')), width = 2000, height = 1600,
#                  res = 300)
#    plot_snpasso(gwas$lod, gwas$snpinfo, main = paste('Brain Score, Chr', chr), chr = chr)
#    abline(h = thr, col = 'red')
#    dev.off()
...
plot_snpasso(gwas_chr4$lod, gwas_chr4$snpinfo, main = paste('RankZ Survival: Chr', chr),
     chr = chr, panel_prop = c(0.25, 0.25, 0.5),genes = genes, xlim = c(start, end), 
     drop_hilit = 1, colors = 'black', show_all_snps = TRUE, sdp_panel = TRUE, 
     strain_labels = LETTERS[1:9])
dev.off()

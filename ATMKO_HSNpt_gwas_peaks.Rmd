...
```{r num_genes}
qtl_rng = GRanges(seqnames = chr2_ts$chr[1], IRanges(start = min(chr2_ts$pos) * 1e6, end = max(chr2_ts$pos) * 1e6))
...
```{r num_genes}
qtl_rng = GRanges(seqnames = chr5_ts$chr[1], IRanges(start = min(chr5_ts$pos) * 1e6, end = max(chr5_ts$pos) * 1e6))
qtl_genes = subsetByOverlaps(genes, qtl_rng)
length(qtl_genes)
```
...
```{r}
# Read in the top SNPs.
chr16_ts = read.csv(file.path(results_dir, 'rz_days_top_snps_chr16.csv'))
```

```{r num_genes}
qtl_rng = GRanges(seqnames = chr16_ts$chr[1], IRanges(start = min(chr16_ts$pos) * 1e6, end = max(chr16_ts$pos) * 1e6))
qtl_genes = subsetByOverlaps(genes, qtl_rng)
length(qtl_genes)
```

```{r}
tbl = table(chr16_ts$ensembl_gene)
sum(tbl[grep('intron', names(tbl))])
sum(tbl[grep('intergenic_variant', names(tbl))])
```
...
```{r}
chr16_ts = subset(chr16_ts, grepl(paste0(csq2keep, collapse = '|'), consequence))
chr16_ts = chr16_ts %>% 
            select(snp_id:alleles, ensembl_gene:X129S1_SvImJ, lod)
```
...
chr16_ts = chr16_ts %>% 
            left_join(e1_data, by = 'gene_id') %>% 
            left_join(e2_data, by = 'gene_id')
write.csv(chr16_ts, file = file.path(results_dir, 'rz_days_snps_expr_chr16.csv'),
          quote = FALSE, row.names = FALSE)
```


```{r chr16_expr}
chr16_expr = data.frame(qtl_genes) %>% 
              select(seqnames:strand, gene_id, gene_name) %>% 
              left_join(e1_data, by = 'gene_id') %>% 
              left_join(e2_data, by = 'gene_id') %>% 
              filter(thymus.x + thymus.y > 0)
